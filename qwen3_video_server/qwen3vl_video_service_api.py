...
# 初始化OpenAI客户端
api_key = "sk-383c293e20ea4163821590ef0bdf06ab"
client = OpenAI(
    # 如果没有配置环境变量，请用阿里云百炼API Key替换：api_key="sk-xxx"
    api_key=api_key,  # 提供空字符串作为默认值，避免启动时出错
...
app = FastAPI(
    ...
    description="基于阿里云百炼Qwen3.5 API的视频内容分析API，支持视频路径和视频帧输入",
    version="1.0.0"
)
...
async def analyze_video(request: VideoAnalysisRequest):
    ...
    try:
        ...
        for i, frame in enumerate(video_frames):
            base64_frame = pil_image_to_base64(frame)
            content.append({
                "type": "image_url",
        ...
        if not api_key:
            # 如果没有设置API密钥，返回模拟结果
            result = "这是一个模拟的视频分析结果。由于未设置API密钥，无法调用真实的AI模型。"
        else:
            # 调用API
